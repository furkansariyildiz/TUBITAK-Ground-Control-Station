...
NAV2D.Navigator = function (options) {
    ...
    if (withOrientation === false) {
        ...
        this.rootObject.addEventListener('dblclick', function (event) {
            ...
            $("#o_x").val(pose.orientation.x);
            $("#o_y").val(pose.orientation.y);
            $("#o_z").val(pose.orientation.z);
            $("#o_w").val(pose.orientation.w);

            $("#p_x").val(pose.position.x);
            $("#p_y").val(pose.position.y);
            $("#p_z").val(pose.position.z);
            $("#position_id").val(0);
            $("#delButton").hide();
            $("#pointModal").modal({ show: true });
            // send the goal
            sendGoal(pose);
        });
